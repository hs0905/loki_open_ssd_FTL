...
void *cpl_memset(void *ptr, int c, uint32_t num){     
    char *p = (char *)ptr;     
    uint32_t i;       
    for(i = 0 ; i < num ; i++){         
        p[i] = c;     
...
void *cpl_memcpy(void *des, const void *src, uint32_t num){
    char *pdes = (char *)des;     
    char *psrc = (char *)src;     
    uint32_t i;       
    for(i = 0 ; i < num ; i++){         
...
void identify_controller(uint32_t pBuffer)
{
	...
	cpl_memset(identifyCNTL, 0, sizeof(ADMIN_IDENTIFY_CONTROLLER));

	identifyCNTL->VID = PCI_VENDOR_ID;
	identifyCNTL->SSVID = PCI_SUBSYSTEM_VENDOR_ID;

	cpl_memset(identifyCNTL->SN, 0x20, 20);
	cpl_memcpy(identifyCNTL->SN, SERIAL_NUMBER, sizeof(SERIAL_NUMBER));

	cpl_memset(identifyCNTL->MN, 0x20, 40);
	cpl_memcpy(identifyCNTL->MN, MODEL_NUMBER, sizeof(MODEL_NUMBER));

	cpl_memset(identifyCNTL->FR, 0x20, 8);
	cpl_memcpy(identifyCNTL->FR, FIRMWARE_REVISION, sizeof(FIRMWARE_REVISION));
