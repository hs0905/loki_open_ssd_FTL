...
{
	uint32_t victimBlockNo, pageNo, virtualSliceAddr, logicalSliceAddr, dieNoForGcCopy, reqSlotTag;
...
uint32_t GetFromGcVictimList(uint32_t dieNo)
{
	...
	cpl_print("[WARNING] There are no free blocks. Abort terminate this ssd. [WARNING]");
	return BLOCK_FAIL;
}
...
{
	uint32_t nextBlock, prevBlock, invalidSliceCnt;
