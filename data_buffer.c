...
{
	uint32_t bufEntry, logicalSliceAddr;
...
{
	uint32_t evictedEntry = dataBufLruList.tailEntry;

	if(evictedEntry == DATA_BUF_NONE)
		cpl_print("[WARNING] There is no valid buffer entry [WARNING]");
...
void SelectiveGetFromDataBufHashList(uint32_t bufEntry)
{
	...
	{
		uint32_t prevBufEntry, nextBufEntry, hashEntry;
