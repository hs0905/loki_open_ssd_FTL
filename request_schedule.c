...
{
	uint32_t readyBusy, wayNo, reqStatus, nextWay, waitWayCnt;
...
void PutToNandWayPriorityTable(uint32_t reqSlotTag, uint32_t chNo, uint32_t wayNo)
{
	if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ)
		...
		cpl_print("[WARNING] wrong reqCode [WARNING]");
...
void IssueNandReq(uint32_t chNo, uint32_t wayNo)
{
	...
	else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ_TRANSFER)
	{
		...
		errorInfo = (uint32_t*)(&eccErrorInfoTablePtr->errorInfo[chNo][wayNo]);
		completion = (uint32_t*)(&completeFlagTablePtr->completeFlag[chNo][wayNo]);
	...
	else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_SET_FEATURE)
	{
		...
		cpl_print("[WARNING] not defined nand req [WARNING]");

}

uint32_t GenerateNandRowAddr(uint32_t reqSlotTag)
{
	uint32_t rowAddr, lun, virtualBlockNo, tempBlockNo, phyBlockNo, tempPageNo, dieNo;
	...
	else if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.nandAddr == REQ_OPT_NAND_ADDR_PHY_ORG)
	{
		...
		cpl_print("[WARNING] wrong nand addr option [WARNING]");
...
uint32_t GenerateDataBufAddr(uint32_t reqSlotTag)
{
	...
	else if(reqPoolPtr->reqPool[reqSlotTag].reqType == REQ_TYPE_NVME_DMA)
	{
		if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_ENTRY)
			...
			cpl_print("[WARNING] wrong reqOpt-dataBufFormat [WARNING]");
	}
	else
		cpl_print("[WARNING] wrong reqType [WARNING]");
...
{
	uint32_t reqSlotTag, completeFlag, statusReport, errorInfo, readyBusy, status;
	uint32_t* statusReportPtr;
	...
	{
		statusReportPtr = (uint32_t*)(&statusReportTablePtr->statusReport[chNo][wayNo]);
	...
	else if(dieStateTablePtr->dieState[chNo][wayNo].reqStatusCheckOpt == REQ_STATUS_CHECK_OPT_NONE)
	{
		...
		cpl_print("[WARNING] wrong request status check option [WARNING]\r\n");
...
{
	uint32_t errorInfo0, errorInfo1, reqSlotTag;
...
{
	uint32_t reqSlotTag, rowAddr, phyBlockNo;
	...
	switch(dieStateTablePtr->dieState[chNo][wayNo].dieState)
	{
		...
		case DIE_STATE_EXE:
			...
			else if(reqStatus == REQ_STATUS_FAIL)
			{
				...
				if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ)
					cpl_print("Read Trigger FAIL on      ");
				else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ_TRANSFER)
					cpl_print("Read Transfer FAIL on     ");
				else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_WRITE)
					cpl_print("Write FAIL on             ");
				// else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_ERASE)
				// 	cpl_print("Erase FAIL on             ");

				rowAddr = GenerateNandRowAddr(reqSlotTag);
				// cpl_print("ch %x way %x rowAddr %x / completion %x statusReport %x \r\n");// chNo, wayNo, rowAddr, completeFlagTablePtr->completeFlag[chNo][wayNo],statusReportTablePtr->statusReport[chNo][wayNo]);
			...
			{
				rowAddr = GenerateNandRowAddr(reqSlotTag);
				cpl_print("ECC Uncorrectable Soon on ch %x way %x rowAddr %x / completion %x statusReport %x \r\n");// chNo, wayNo, rowAddr, completeFlagTablePtr->completeFlag[chNo][wayNo],statusReportTablePtr->statusReport[chNo][wayNo]);
				...
				cpl_print("[WARNING] wrong req status [WARNING]");
			break;
	}
}
